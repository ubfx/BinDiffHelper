...
public class UpdatePlateCommentsAction extends DockingAction {
	...
	/**
	 * Take a string, split it apart and replace a string prefixed by the value in
	 * key with the value in rep
	 * <p>
	 * This is a utility function I use to update comments for updating
	 * plate-comments at the top of functions
	 * <p>
	 ...
	 * @return A string with the line matching key replaced. If no line matched,
	 *         then we return rep
	 */
	...
	public void actionPerformed(ActionContext arg0) {
		...
		for (ComparisonTableModel.Entry e : entries) {
			...
			if (symtab.hasSymbol(e.primaryAddress)) {
				...
				if (f == null) // skip this if it is a symbol, but not a function
					continue;

				String origComment = f.getComment();
				String newBindiffComment = String.format("*** %2f%% match with %2f%% confidence using %s ***",
						e.similarity * 100, e.confidence * 100, e.algorithm);
				String newFunctionLinkComment = String.format("*** %s@%08x {@program %s@%08x} ***",
						e.secondaryFunctionName, e.secondaryAddress, plugin.provider.otherProg, e.secondaryAddress);
